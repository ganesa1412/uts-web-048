<?php 
	class Stand_Order{
		...
		function getActiveOrder($id){
			$data = mysqli_query($this->conn, "SELECT * FROM tb_order WHERE status = 1 AND id_stand = $id");
		...
		function setStatus($id, $status){
			$ed = mysqli_query($this->conn, "UPDATE tb_order SET status = '$status' WHERE id_order = $id");
			return $ed;
		}
